...
\begin{itemize}
  \item GTK+ 2.6.x      ftp://ftp.gtk.org/pub/gtk/v2.6/
  \item PyGTK 2.6.x     ftp://ftp.gtk.org/pub/gtk/python/v2.6/
  \item Python > 2.3    http://www.python.org/download/
  \item gazpacho 0.6.x  http://ftp.gnome.org/pub/GNOME/sources/gazpacho/0.6/
...
\begin{itemize}
  \item simple: o mínimo necessário. O modelo é um objeto python
    convencional.
  \item pickle: uma versão que persiste os dados utilizando o formato de
    serialização pickle.
  \item sqlobject: uma versão que persiste os dados utilizando sqlobject.
  \item kiwi-model: uma versão que utiliza kiwi.models.Model. Esta versão
...
\begin{itemize}
  \item Visão (Views): representação gráfica, com componentes de interface
    de usuário: botões, entrada de texto, rótulos, figuras e outros.
  \item Controladores (Controllers): trata os sinais e eventos gerados pelo
    usuário que interage com os componentes de Visão: entradas de
    teclado e mouse, eventos temporais e outros.
  \item Delegadores (Delegates): classes que combinam Visão e
    Controladores para facilitar e agilizar o desenvolvimento.
  \item Modelos (Models): são objetos que representam/modelam seus dados.
  \item Proxies: facilitador que mantém sincronizados os dados do modelo e
...
\begin{itemize}
  \item SlaveView: é considerado um objeto do GTK e pode ser embutido em
    ...
    gazpacho/glade.
  \item BaseView: derivado de SlaveView, implementa algumas funcionalidades
    extra necessárias para uma janela principal (toplevel). O parâmetro
    do construtor \lstinline|delete_handler| é um objeto "chamável"
    (\textit{callable}) que
...
\begin{enumerate}
  \item [Gazpacho] Desenhe os seus componentes em arquivos Glade/XML
    separados.
  \item [Python] Crie uma instância de SlaveView para cada componente criado.
  \item [Gazpacho] Na tela principal, na qual os objetos serão embutidos,
    ...
    lugar.
  \item [Python] Crie uma instância de BaseView para a tela principal.
  \item [Python] Utilize o método \lstinline|attach_slave()| de BaseView com os
...
\lstinline|entry_editor| e \lstinline|list_entries| no componente
principal addressbook. Note,
...
(sinal\_do\_componente="content_changed"), poderíamos usar:
...
rótulo "label\_last\_button". A caixa de texto, ao ser modificada, irá
ter seu conteúdo copiado para o rótulo "label\_name". Note que todos os
...
Addressbook trata este sinal com \lstinline|entry_selected()|, perceba
